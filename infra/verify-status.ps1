...
try {
  ...
  if ($title -match "Umami") { Write-Host "  FAIL: still Umami — run infra/uptime-kuma setup + Cloudflare Tunnel" -ForegroundColor Red }
  elseif ($title -match "Uptime Kuma|Dashboard") { Write-Host "  OK: Uptime Kuma" -ForegroundColor Green }
...
Write-Host "Checking upwatch.online admin bundle …"
try {
